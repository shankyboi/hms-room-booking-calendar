...
public Guid medicationtype	{ get; set; }
...
[xssFilter]
			...
			public class MedicineModelValidator: AbstractValidator<MedicineModel>
			{
					...
					public MedicineModelValidator()
					{
						 ...
						 When(model => model.craftmyapp_actionmethodname == "Add_Medicine", () =>
                                    {
                                        {RuleFor(m => m.medicationtype)
.NotEmpty().WithMessage("Medication Type is required")
;
RuleFor(m => m.medicinename)
.NotEmpty().WithMessage("Medicine Name is required")
.MaximumLength(128).WithMessage("The allowed length of Medicine Name is 128 characters or fewer")
;
RuleFor(m => m.price)
...
When(model => model.craftmyapp_actionmethodname == "Update_Medicine", () =>
                                    {
                                        {RuleFor(m => m.medicationtype)
.NotEmpty().WithMessage("Medication Type is required")
;
