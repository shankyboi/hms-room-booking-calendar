			  ...
			  CREATE OR REPLACE FUNCTION  "get_all_TherapyCategory"
              (
...
,TherapyCategory.categorydescription
			  ...
			  WHERE
               (lvar_tenantid is null or COALESCE(cast(TherapyCategory.tenantid as varchar),'') = Any(lvar_tenantid))
                 
			   AND TherapyCategory.isdeleted=false
			  ;
