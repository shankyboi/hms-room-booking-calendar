			  ...
			  CREATE OR REPLACE FUNCTION  "get_all_ActionType"
              (
...
,ActionType.description
			  ...
			  FROM ActionType
			  WHERE
               (lvar_tenantid is null or COALESCE(cast(ActionType.tenantid as varchar),'') = Any(lvar_tenantid))
                 
			   AND ActionType.isdeleted=false
			  
		        AND (((pvar_searchterm is null) or COALESCE(ActionType.ActionTypeid::varchar,'') ilike pvar_searchterm) OR ((pvar_searchterm is null) or COALESCE(ActionType.actiontype,'') ilike pvar_searchterm) OR ((pvar_searchterm is null) or COALESCE(ActionType.description,'') ilike pvar_searchterm)) 
