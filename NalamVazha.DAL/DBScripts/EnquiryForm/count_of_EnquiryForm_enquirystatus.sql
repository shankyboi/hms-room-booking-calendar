			  ...
			  CREATE OR REPLACE FUNCTION  "count_of_EnquiryForm_enquirystatus"
...
,pvar_enquirystatus Varchar(1024)
			  ...
			  RETURNS TABLE(count bigint,enquirystatus varchar)
			  AS $BODY$
              ...
              RETURN QUERY
			  SELECT  
			  Count(*),EnquiryForm.enquirystatus::varchar as enquirystatus
			  FROM EnquiryForm
...
AND (pvar_emailaddress is null or LENGTH(CAST(pvar_emailaddress as Varchar))=0 or CAST(EnquiryForm.emailaddress as VARCHAR)=pvar_emailaddress)
               ...
               GROUP BY EnquiryForm.enquirystatus;
