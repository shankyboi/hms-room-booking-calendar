CREATE OR REPLACE FUNCTION public."Added_Enquiries"(
	pvar_tenantid character varying,
	pvar_enquirynumber character varying,
	pvar_patientname character varying,
	pvar_phonenumber character varying,
	pvar_emailaddress character varying,
	pvar_enquirystatus character varying,
...
AS $BODY$
              ...
              declare lvar_tenantid varchar[];declare lstr_usersid varchar;  declare lstr_userrole varchar;
               
          	  BEGIN
			  /*This code generated from tDev Powered by Mahat, Build Number :#2024-01-001(Updated on 06-01-2024 12:57PM) on 03/10/2026 05:47:40*/
			  		
                SELECT  SPLIT_PART(pvar_tenantid, '|', 1),SPLIT_PART(pvar_tenantid, '|', 2) into lstr_usersid,pvar_tenantid;

SELECT userrole INTO lstr_userrole
...
LEFT OUTER JOIN PatientCategory ____PatientCategory ON EnquiryForm.enquirertype=____PatientCategory.PatientCategoryid
...
AND (
    lstr_userrole <> 'Health Seeker'
    OR
    (
        lstr_userrole = 'Health Seeker'
        AND EnquiryForm.createduser::varchar = lstr_usersid
        AND EnquiryForm.createduser <> '00000000-0000-0000-0000-000000000000'
    )
)
AND (
    pvar_createddate_automatonfrom IS NULL
    OR LENGTH(TRIM(pvar_createddate_automatonfrom)) = 0
    OR EnquiryForm.createddate >= pvar_createddate_automatonfrom::timestamp
)

AND (
    pvar_createddate_automatonto IS NULL
    OR LENGTH(TRIM(pvar_createddate_automatonto)) = 0
    OR EnquiryForm.createddate < pvar_createddate_automatonto::timestamp
)
AND (pvar_enquirynumber is null or pvar_enquirynumber ='0' or LENGTH(CAST(pvar_enquirynumber as Varchar))=0 or CAST(EnquiryForm.enquirynumber as VARCHAR)=pvar_enquirynumber)
AND (pvar_patientname is null or pvar_patientname ='0' or LENGTH(CAST(pvar_patientname as Varchar))=0 or CAST(EnquiryForm.patientname as VARCHAR)=pvar_patientname)
AND (pvar_phonenumber is null or pvar_phonenumber ='0' or LENGTH(CAST(pvar_phonenumber as Varchar))=0 or CAST(EnquiryForm.phonenumber as VARCHAR)=pvar_phonenumber)
AND (pvar_emailaddress is null or pvar_emailaddress ='0' or LENGTH(CAST(pvar_emailaddress as Varchar))=0 or CAST(EnquiryForm.emailaddress as VARCHAR)=pvar_emailaddress)
...
AND (pvar_verifiedstatus is null or pvar_verifiedstatus ='0' or LENGTH(CAST(pvar_verifiedstatus as Varchar))=0 or CAST(EnquiryForm.verifiedstatus as VARCHAR)=pvar_verifiedstatus)
 AND COALESCE(EnquiryForm.verifiedstatus,'') !='Revised'
 ...
 OR ((pvar_searchterm is null) or CAST(____PatientCategory.patientcategoryname AS VARCHAR) ilike pvar_searchterm)
))                   
                    ,'detail'
...
,EnquiryForm.enquirertype
,CAST(____PatientCategory.patientcategoryname AS VARCHAR) as enquirertype_master
...
WHEN 'enquirertype' THEN ____PatientCategory.patientcategoryname::TEXT
...
ALTER FUNCTION public."Added_Enquiries"(pvar_tenantid character varying, pvar_enquirynumber character varying, pvar_patientname character varying, pvar_phonenumber character varying, pvar_emailaddress character varying, pvar_enquirystatus character varying, pvar_verifiedstatus character varying, pvar_pagesize integer, pvar_pagenumber integer, pvar_searchterm character varying, pvar_sort_fields json)
