
CREATE OR REPLACE FUNCTION public."Remove_People_Time_Preference"(
	pvar_peopletimepreferenceid character varying,
	pvar_modifieduser character varying,
	OUT pvar_returnmessage character varying)
    RETURNS character varying
    LANGUAGE 'plpgsql'
    COST 100
    VOLATILE PARALLEL UNSAFE
AS $BODY$
  
              DECLARE lv_viewactionroles Varchar(128);
              BEGIN
              /*This code generated from tDev Powered by Mahat, Build Number :#2024-01-001(Updated on 06-01-2024 12:57PM) on 03/23/2026 14:00:31*/
			  IF "Check_Authorization"(pvar_modifieduser::uuid, 'PeopleTimePreference', 'delete') THEN
			  
            INSERT INTO history
VALUES('PeopleTimePreference', NOW(),
(SELECT query_to_xml('SELECT * FROM PeopleTimePreference WHERE CAST(PeopleTimePreference.PeopleTimePreferenceid AS VARCHAR)= '''||pvar_PeopleTimePreferenceid||'''', true, false, '')));

			 INSERT INTO history
VALUES('PeopleTimePreference_timepreference', NOW(),
(SELECT query_to_xml('SELECT * FROM PeopleTimePreference_timepreference WHERE PeopleTimePreference_timepreference.PeopleTimePreferenceid= '''||pvar_PeopleTimePreferenceid||'''', true, false, '')));

			 UPDATE PeopleTimePreference SET
													isdeleted=true ,modifieduser=CAST(pvar_modifieduser AS UUID),modifieddate=NOW()
													WHERE CAST(PeopleTimePreference.PeopleTimePreferenceid AS VARCHAR)=pvar_PeopleTimePreferenceid;
					 
				  pvar_returnMessage := '201.1';
					 
																ELSE
																

															
																INSERT INTO system_logging
																(
																Log_code
																,system_logging_guid
																,log_application
																,log_date
																,log_level
																,log_logger
																,log_message
																,log_user_name
																)
																VALUES
																('401.1'
																,gen_random_uuid()
																,'Store Proc Authorization Check'
																,NOW()
																,'Critical'
																,'Remove_People_Time_Preference'
																,'Authorization Failed Remove_People_Time_Preference'
																,pvar_modifieduser
																);
																pvar_returnMessage := '401.1';
																
																END IF;
			  
                    /*EXCEPTION WHEN OTHERS THEN
			 
						INSERT INTO system_logging
						(
						Log_code
						,system_logging_guid
						,log_application
						,log_date
						,log_level
						,log_logger
						,log_message
						)
						VALUES
						('16'
						,gen_random_uuid()
						,'Postgre Function Exception'
						,NOW()
						,'16'
						,'Remove_People_Time_Preference'
						,'user delete failed'
						);				 
					    pvar_returnMessage := 'user delete failed';*/
				  
			  
 			  END
              
$BODY$;

