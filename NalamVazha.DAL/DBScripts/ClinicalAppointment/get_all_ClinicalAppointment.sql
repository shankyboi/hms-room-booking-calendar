			  ...
			  CREATE OR REPLACE FUNCTION  "get_all_ClinicalAppointment"
              (
...
,ClinicalAppointment.tokennumber
			  ...
			  WHERE
               (lvar_tenantid is null or COALESCE(cast(ClinicalAppointment.tenantid as varchar),'') = Any(lvar_tenantid))
                 
			   AND ClinicalAppointment.isdeleted=false
			  
		        AND (((pvar_searchterm is null) or COALESCE(ClinicalAppointment.ClinicalAppointmentid::varchar,'') ilike pvar_searchterm) OR ((pvar_searchterm is null) or COALESCE(ClinicalAppointment.bookingid,'') ilike pvar_searchterm) OR ((pvar_searchterm is null) or COALESCE(ClinicalAppointment.tokennumber,'') ilike pvar_searchterm)) 
