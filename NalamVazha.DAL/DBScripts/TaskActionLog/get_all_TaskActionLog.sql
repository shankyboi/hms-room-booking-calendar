			  ...
			  CREATE OR REPLACE FUNCTION  "get_all_TaskActionLog"
              (
...
,TaskActionLog.description
			  ...
			  FROM TaskActionLog
			  WHERE
               (lvar_tenantid is null or COALESCE(cast(TaskActionLog.tenantid as varchar),'') = Any(lvar_tenantid))
                 
			   AND TaskActionLog.isdeleted=false
			  
		        AND (((pvar_searchterm is null) or COALESCE(TaskActionLog.TaskActionLogid::varchar,'') ilike pvar_searchterm) OR ((pvar_searchterm is null) or COALESCE(TaskActionLog.comments,'') ilike pvar_searchterm) OR ((pvar_searchterm is null) or COALESCE(TaskActionLog.summary,'') ilike pvar_searchterm) OR ((pvar_searchterm is null) or COALESCE(TaskActionLog.description,'') ilike pvar_searchterm)) 
