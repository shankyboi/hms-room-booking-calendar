								...
								CREATE OR REPLACE FUNCTION  "lookup_Finance_paymentstatus"
								(
...
,paymentstatus Varchar
	...
	then
		...
		pvar_searchterm := null;
                                ...
                                RETURN QUERY        
								SELECT DISTINCT CAST(bp.paymentstatus AS varchar) AS BillingPaymentid, CAST(bp.paymentstatus AS varchar) AS paymentstatus
								FROM BillingPayment bp
								 WHERE COALESCE(cast(bp.tenantid as varchar),'') = Any(lvar_tenantid) AND bp.isdeleted=false AND NULLIF(trim(bp.paymentstatus), '') IS NOT NULL
AND (COALESCE(pvar_patientname, '') = '' OR CAST(bp.patientname AS varchar) = pvar_patientname)
AND ((pvar_searchterm is null) OR bp.paymentstatus ilike pvar_searchterm)
 ORDER BY 2 ASC
                                 limit pvar_pagesize
