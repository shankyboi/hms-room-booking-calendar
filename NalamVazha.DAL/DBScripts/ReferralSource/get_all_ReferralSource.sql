			  ...
			  CREATE OR REPLACE FUNCTION  "get_all_ReferralSource"
              (
...
,ReferralSource.websiteurl
			  ...
			  FROM ReferralSource
			  WHERE
               (lvar_tenantid is null or COALESCE(cast(ReferralSource.tenantid as varchar),'') = Any(lvar_tenantid))
                 
			   AND ReferralSource.isdeleted=false
			  ;
