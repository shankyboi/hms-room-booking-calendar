			  ...
			  CREATE OR REPLACE FUNCTION  "Referral_Source_List"
              (pvar_tenantid Varchar
)
			  RETURNS TABLE(tenantid uuid
,_tenantName Varchar(128)
,ReferralSourceid uuid
,referralsourcename Varchar,contactnumber Varchar,websiteurl Varchar,createduser uuid
...
,ReferralSource.websiteurl
 ...
 LEFT OUTER JOIN tenant ON ReferralSource.tenantid=tenant.tenantid
				...
				WHERE (lvar_tenantid is null or COALESCE(cast(ReferralSource.tenantid as varchar), '') = Any(lvar_tenantid)) AND ReferralSource.isdeleted=false

				 ORDER BY ReferralSource.createddate DESC;
