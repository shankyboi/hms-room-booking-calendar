			  ...
			  CREATE OR REPLACE FUNCTION  "Remove_Unit_of_Measure"
			  (
				  pvar_UnitofMeasureid Varchar(50)
			  ...
			  IF "Check_Authorization"(pvar_modifieduser::uuid, 'UnitofMeasure', 'delete') THEN
			  
            INSERT INTO history
VALUES('UnitofMeasure', NOW(),
(SELECT query_to_xml('SELECT * FROM UnitofMeasure WHERE CAST(UnitofMeasure.UnitofMeasureid AS VARCHAR)= '''||pvar_UnitofMeasureid||'''', true, false, '')));
			 ...
			 UPDATE UnitofMeasure SET
				  ...
				  pvar_returnMessage := '201.1';
																...
																,NOW()
																,'Critical'
																,'Remove_Unit_of_Measure'
																,'Authorization Failed Remove_Unit_of_Measure'
																,pvar_modifieduser
																);
