			  ...
			  CREATE OR REPLACE FUNCTION  "get_all_ClinicalTask"
              (
...
,ClinicalTask.competency
			  ...
			  FROM ClinicalTask
			  WHERE
               (lvar_tenantid is null or COALESCE(cast(ClinicalTask.tenantid as varchar),'') = Any(lvar_tenantid))
                 
			   AND ClinicalTask.isdeleted=false
			  ;
