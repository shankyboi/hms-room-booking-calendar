			  ...
			  CREATE OR REPLACE FUNCTION  "Remove_Room_Type"
			  (
			  ...
			  IF "Check_Authorization"(pvar_modifieduser::uuid, 'RoomType', 'delete') THEN
			  
            INSERT INTO history
VALUES('RoomType', NOW(),
(SELECT query_to_xml('SELECT * FROM RoomType WHERE CAST(RoomType.RoomTypeid AS VARCHAR)= '''||pvar_RoomTypeid||'''', true, false, '')));
...
(SELECT query_to_xml('SELECT * FROM RoomType_refundpolicy WHERE RoomType_refundpolicy.RoomTypeid= '''||pvar_RoomTypeid||'''', true, false, '')));
			 ...
			 UPDATE RoomType SET
				  ...
				  pvar_returnMessage := '201.1';
																...
																,'Critical'
																,'Remove_Room_Type'
																,'Authorization Failed Remove_Room_Type'
																,pvar_modifieduser
																);
