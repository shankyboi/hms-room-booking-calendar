								        ...
								        CREATE OR REPLACE FUNCTION  "lookup_ShiftPlanning_people_personname"
								        (
...
,workprofile Varchar
	...
	then
		...
		pvar_searchterm := null;
                                        ...
                                        RETURN QUERY        
								        SELECT  
								        CAST(People.Peopleid AS Varchar) as Peopleid,CAST(People.firstname AS Varchar) as firstname,CAST(People.lastname AS Varchar) as lastname,CAST(People.workprofile AS Varchar) as workprofile
								        FROM People
 ...
 AND (pvar_workprofile IS NOT NULL AND CAST(People.workprofile AS Varchar) = pvar_workprofile)
