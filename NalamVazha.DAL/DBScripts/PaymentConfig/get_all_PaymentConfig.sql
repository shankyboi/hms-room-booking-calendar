			  ...
			  CREATE OR REPLACE FUNCTION  "get_all_PaymentConfig"
              (
...
,tenantid uuid
			  ...
			  FROM PaymentConfig
			  WHERE
               (lvar_tenantid is null or COALESCE(cast(PaymentConfig.tenantid as varchar),'') = Any(lvar_tenantid))
                 
			   AND PaymentConfig.isdeleted=false
			  ;
