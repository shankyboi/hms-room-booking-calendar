			  ...
			  CREATE OR REPLACE FUNCTION  "Remove_Payment_Config"
			  (
			  ...
			  IF "Check_Authorization"(pvar_modifieduser::uuid, 'PaymentConfig', 'delete') THEN
			  
            INSERT INTO history
VALUES('PaymentConfig', NOW(),
(SELECT query_to_xml('SELECT * FROM PaymentConfig WHERE CAST(PaymentConfig.PaymentConfigid AS VARCHAR)= '''||pvar_PaymentConfigid||'''', true, false, '')));
...
(SELECT query_to_xml('SELECT * FROM PaymentConfig_keyinfo WHERE PaymentConfig_keyinfo.PaymentConfigid= '''||pvar_PaymentConfigid||'''', true, false, '')));
			 ...
			 UPDATE PaymentConfig SET
				  ...
				  pvar_returnMessage := '201.1';
																...
																,NOW()
																,'Critical'
																,'Remove_Payment_Config'
																,'Authorization Failed Remove_Payment_Config'
																,pvar_modifieduser
																);
