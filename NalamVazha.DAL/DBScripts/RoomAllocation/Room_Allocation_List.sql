			  ...
			  CREATE OR REPLACE FUNCTION  "Room_Allocation_List"
              (pvar_tenantid Varchar
,pvar_ipdno Varchar
,pvar_block Varchar
,pvar_building Varchar
,pvar_floor Varchar
,pvar_room Varchar
...
,pvar_sort_fields json
			  ...
			  /*This code generated from tDev Powered by Mahat, Build Number :#2024-01-001(Updated on 06-01-2024 12:57PM) on 03/10/2026 09:43:10*/
                ...
                lvar_tenantid := lvar_tenantid || ARRAY[''::character varying] || ARRAY['00000000-0000-0000-0000-000000000000'::character varying];
                    ...
                    pvar_ipdno := NULLIF(pvar_ipdno, '');
                    pvar_block := NULLIF(pvar_block, '');
                    pvar_building := NULLIF(pvar_building, '');
                    pvar_floor := NULLIF(pvar_floor, '');
                    pvar_room := NULLIF(pvar_room, '');
...
INNER JOIN Room _____Room ON RoomAllocation.room=_____Room.Roomid
 ...
 AND ((pvar_ipdno is null) or CAST(COALESCE(NULLIF(RoomAllocation.ipdno, '00000000-0000-0000-0000-000000000000'::uuid), _RoomAllocationIpd.ipdno) AS VARCHAR) = pvar_ipdno)
 AND ((pvar_block is null) or CAST(RoomAllocation.block AS VARCHAR) = pvar_block)
 AND ((pvar_building is null) or CAST(RoomAllocation.building AS VARCHAR) = pvar_building)
 AND ((pvar_floor is null) or CAST(RoomAllocation.floor AS VARCHAR) = pvar_floor)
 AND ((pvar_room is null) or CAST(RoomAllocation.room AS VARCHAR) = pvar_room)
