								...
								CREATE OR REPLACE FUNCTION  "lookup_DailyTask_opdreferencenumber"
								(
...
,bookingreferencenumber Varchar
	...
	then
		...
		pvar_searchterm := null;
                                ...
                                RETURN QUERY        
								SELECT  
								CAST(OPDForm.OPDFormid AS Varchar) as OPDFormid,CAST(OPDForm.bookingreferencenumber AS Varchar) as bookingreferencenumber
								FROM OPDForm
...
AND (pvar_patientname IS NOT NULL AND CAST( OPDForm.patientname AS Varchar) = pvar_patientname)
 ...
 AND  ((pvar_searchterm is null)  or OPDForm.bookingreferencenumber::varchar ilike pvar_searchterm or  CAST(OPDForm.OPDFormid AS Varchar) ilike pvar_searchterm)
 ORDER BY OPDForm.bookingreferencenumber ASC
                                 limit pvar_pagesize
