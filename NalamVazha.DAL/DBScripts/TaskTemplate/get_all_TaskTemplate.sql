			  ...
			  CREATE OR REPLACE FUNCTION  "get_all_TaskTemplate"
              (
...
,TaskTemplate.duration
			  ...
			  FROM TaskTemplate
			  WHERE
               (lvar_tenantid is null or COALESCE(cast(TaskTemplate.tenantid as varchar),'') = Any(lvar_tenantid))
                 
			   AND TaskTemplate.isdeleted=false
			  
		        AND (((pvar_searchterm is null) or COALESCE(TaskTemplate.TaskTemplateid::varchar,'') ilike pvar_searchterm) OR ((pvar_searchterm is null) or COALESCE(TaskTemplate.taskname,'') ilike pvar_searchterm)) 
