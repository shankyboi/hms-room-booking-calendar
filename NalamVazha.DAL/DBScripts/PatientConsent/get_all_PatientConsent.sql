			  ...
			  CREATE OR REPLACE FUNCTION  "get_all_PatientConsent"
              (
...
,PatientConsent.consentfile
			  ...
			  FROM PatientConsent
			  WHERE
               (lvar_tenantid is null or COALESCE(cast(PatientConsent.tenantid as varchar),'') = Any(lvar_tenantid))
                 
			   AND PatientConsent.isdeleted=false
			  ;
