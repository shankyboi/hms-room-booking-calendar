			  ...
			  CREATE OR REPLACE FUNCTION  "get_all_PatientVisit"
              (
...
,PatientVisit.notes
			  ...
			  FROM PatientVisit
			  WHERE
               (lvar_tenantid is null or COALESCE(cast(PatientVisit.tenantid as varchar),'') = Any(lvar_tenantid))
                 
			   AND PatientVisit.isdeleted=false
			  
		        AND (((pvar_searchterm is null) or COALESCE(PatientVisit.PatientVisitid::varchar,'') ilike pvar_searchterm) OR ((pvar_searchterm is null) or COALESCE(PatientVisit.visitnumber,'') ilike pvar_searchterm) OR ((pvar_searchterm is null) or COALESCE(PatientVisit.notes,'') ilike pvar_searchterm)) 
