								...
								CREATE OR REPLACE FUNCTION  "lookup_Appoinment_doctor"
								(
...
,practitioner Varchar
	...
	then
		...
		pvar_searchterm := null;
                                ...
                                RETURN QUERY        
								SELECT DISTINCT
								CAST(COALESCE(ca.actualpractitioner, ca.practitioner) AS varchar) AS ClinicalAppointmentid,
								CAST(NULLIF(btrim(concat_ws(' ', NULLIF(p.firstname, ''), NULLIF(p.lastname, ''))), '') AS varchar) AS practitioner
								FROM ClinicalAppointment ca
								JOIN People p ON COALESCE(ca.actualpractitioner, ca.practitioner) = p.Peopleid
								 WHERE COALESCE(cast(ca.tenantid as varchar),'') = Any(lvar_tenantid) AND ca.isdeleted=false
 AND COALESCE(p.status, 'Active')='Active'
 AND ((pvar_searchterm is null) OR concat_ws(' ', p.firstname, p.lastname) ilike pvar_searchterm)
 ORDER BY 2 ASC
                                 limit pvar_pagesize
