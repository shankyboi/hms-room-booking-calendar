			  ...
			  CREATE OR REPLACE FUNCTION  "get_all_Appoinment"
              (
...
,Appoinment.status
			  ...
			  FROM Appoinment
			  WHERE
               (lvar_tenantid is null or COALESCE(cast(Appoinment.tenantid as varchar),'') = Any(lvar_tenantid))
                 
			   AND Appoinment.isdeleted=false
			  
		        AND (((pvar_searchterm is null) or COALESCE(Appoinment.Appoinmentid::varchar,'') ilike pvar_searchterm)) 
