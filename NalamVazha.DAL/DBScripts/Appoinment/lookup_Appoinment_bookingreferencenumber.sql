								...
								CREATE OR REPLACE FUNCTION  "lookup_Appoinment_bookingreferencenumber"
								(
...
,opdnumber Varchar
	...
	then
		...
		pvar_searchterm := null;
                                ...
                                RETURN QUERY        
								SELECT DISTINCT
								CAST(resolved.bookingreference AS varchar) AS ClinicalAppointmentid,
								CAST(resolved.bookingreference AS varchar) AS ipdnumber,
								''::varchar AS opdnumber
								...
								LEFT JOIN IPDApplicationForm ipd
								 ...
								 AND (ca.bookingid = ipd.IPDApplicationFormid::varchar OR ca.bookingid = ipd.bookingreferencenumber)
								...
								LEFT JOIN OPDForm opd
								 ...
								 AND (ca.bookingid = opd.OPDFormid::varchar OR ca.bookingid = opd.bookingreferencenumber)
								 AND COALESCE(opd.isdeleted, false) = false
								CROSS JOIN LATERAL (SELECT COALESCE(ipd.bookingreferencenumber, opd.bookingreferencenumber, ca.bookingid)::varchar AS bookingreference) resolved
								 WHERE COALESCE(cast(ca.tenantid as varchar),'') = Any(lvar_tenantid) AND ca.isdeleted=false
 AND NULLIF(trim(resolved.bookingreference), '') IS NOT NULL
 AND ((pvar_searchterm is null) OR resolved.bookingreference ilike pvar_searchterm)
 ORDER BY 2 ASC
                                 limit pvar_pagesize
