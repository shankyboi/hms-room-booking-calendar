								...
								CREATE OR REPLACE FUNCTION  "lookup_Appoinment_origin"
								(
...
,origin Varchar
	...
	then
		...
		pvar_searchterm := null;
                                ...
                                RETURN QUERY        
								SELECT DISTINCT
								CAST(ca.origin AS varchar) AS ClinicalAppointmentid, CAST(ca.origin AS varchar) AS origin
								FROM ClinicalAppointment ca
								 WHERE COALESCE(cast(ca.tenantid as varchar),'') = Any(lvar_tenantid) AND ca.isdeleted=false AND NULLIF(trim(ca.origin), '') IS NOT NULL
 AND ((pvar_searchterm is null) OR ca.origin ilike pvar_searchterm)
 ORDER BY 2 ASC
                                 limit pvar_pagesize
