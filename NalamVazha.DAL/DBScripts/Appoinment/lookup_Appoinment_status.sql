								...
								CREATE OR REPLACE FUNCTION  "lookup_Appoinment_status"
								(
...
,status Varchar
	...
	then
		...
		pvar_searchterm := null;
                                ...
                                RETURN QUERY        
								SELECT DISTINCT
								CAST(ca.status AS varchar) AS ClinicalAppointmentid, CAST(ca.status AS varchar) AS status
								FROM ClinicalAppointment ca
								 WHERE COALESCE(cast(ca.tenantid as varchar),'') = Any(lvar_tenantid) AND ca.isdeleted=false AND NULLIF(trim(ca.status), '') IS NOT NULL
 AND ((pvar_searchterm is null) OR ca.status ilike pvar_searchterm)
 ORDER BY 2 ASC
                                 limit pvar_pagesize
