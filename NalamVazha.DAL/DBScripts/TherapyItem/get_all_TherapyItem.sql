			  ...
			  CREATE OR REPLACE FUNCTION  "get_all_TherapyItem"
              (
...
,TherapyItem.therapyitemimage
			  ...
			  FROM TherapyItem
			  WHERE
               (lvar_tenantid is null or COALESCE(cast(TherapyItem.tenantid as varchar),'') = Any(lvar_tenantid))
                 
			   AND TherapyItem.isdeleted=false
			  ;
