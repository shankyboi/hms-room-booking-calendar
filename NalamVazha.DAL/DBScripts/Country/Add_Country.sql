
			  CREATE OR REPLACE FUNCTION  "Add_Country"
			  (
				  pvar_Countryid uuid
,
pvar_countryname Varchar(128)
,
pvar_countrycode Varchar(128)
,
pvar_countryshortcode Varchar(128)
 
				  ,pvar_createduser  uuid 

				  ,OUT pvar_returnMessage Varchar(4000)
			  )
			  RETURNS Varchar(4000) 
              AS $BODY$  
              DECLARE lv_viewactionroles Varchar(128);
              BEGIN

				/*This code generated from tDev Powered by Mahat, Build Number :#2024-01-001(Updated on 06-01-2024 12:57PM) on 03/10/2026 09:40:31*/
		

			  
                                                                                    if pvar_Countryid is null then
                                                                                    pvar_Countryid:=gen_random_uuid();
                                                                                    end if;	
                                                                                    
			  
			  IF "Check_Authorization"(pvar_createduser, 'Country', 'create') THEN
			  pvar_returnMessage:='';
			  
                
			  if(pvar_returnMessage='')
			  THEN

			  

			  INSERT INTO Country(
				 countryname
,countrycode
,countryshortcode

				 ,createduser
				 ,Countryid
				 
                
			  )
			  VALUES (
 				 pvar_countryname
,pvar_countrycode
,pvar_countryshortcode

				 ,pvar_createduser
				 ,pvar_Countryid
				 
                   
			  );
			   
               

			  


			  
					 
			  pvar_returnMessage :='201.1';
               
              END IF;
			   

			  
																ELSE
																

															
																INSERT INTO system_logging
																(
																Log_code
																,system_logging_guid
																,log_application
																,log_date
																,log_level
																,log_logger
																,log_message
																,log_user_name
																)
																VALUES
																('401.1'
																,gen_random_uuid()
																,'Store Proc Authorization Check'
																,NOW()
																,'Critical'
																,'Add_Country'
																,'Authorization Failed Add_Country'
																,pvar_createduser
																);
																pvar_returnMessage := '401.1';
																
																END IF;
			  /*EXCEPTION WHEN OTHERS THEN
			 
						INSERT INTO system_logging
						(
						Log_code
						,system_logging_guid
						,log_application
						,log_date
						,log_level
						,log_logger
						,log_message
						)
						VALUES
						('16'
						,gen_random_uuid()
						,'Store Proc Exception'
						,NOW()
						,'16'
						,'Add_Country'
						,'insert failed'
						);
                        pvar_returnMessage := 'Add_Country - Insert failed';*/
			  	
			  END
              $BODY$
              LANGUAGE plpgsql;

