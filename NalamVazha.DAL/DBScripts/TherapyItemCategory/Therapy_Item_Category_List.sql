			  ...
			  CREATE OR REPLACE FUNCTION  "Therapy_Item_Category_List"
              (pvar_tenantid Varchar
)
			  RETURNS TABLE(tenantid uuid
,_tenantName Varchar(128)
,TherapyItemCategoryid uuid
,itemcategory Varchar,description Varchar,createduser uuid
...
,TherapyItemCategory.description
 ...
 LEFT OUTER JOIN tenant ON TherapyItemCategory.tenantid=tenant.tenantid
				...
				WHERE (lvar_tenantid is null or COALESCE(cast(TherapyItemCategory.tenantid as varchar), '') = Any(lvar_tenantid)) AND TherapyItemCategory.isdeleted=false

				 ORDER BY TherapyItemCategory.createddate DESC;
