			  ...
			  CREATE OR REPLACE FUNCTION  "get_all_TherapyItemCategory"
              (
...
,TherapyItemCategory.description
			  ...
			  WHERE
               (lvar_tenantid is null or COALESCE(cast(TherapyItemCategory.tenantid as varchar),'') = Any(lvar_tenantid))
                 
			   AND TherapyItemCategory.isdeleted=false
			  ;
