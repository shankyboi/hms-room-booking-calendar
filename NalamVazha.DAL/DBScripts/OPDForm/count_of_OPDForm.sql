			  ...
			  CREATE OR REPLACE FUNCTION  "count_of_OPDForm"
...
,pvar_patientname Varchar(1024)
              ...
              RETURN QUERY
			  SELECT  
			  Count(*),OPDForm.verifiedstatus 
			  FROM OPDForm
			  WHERE (lvar_tenantid is null or COALESCE(cast(OPDForm.tenantid as varchar), '') = Any(lvar_tenantid)) AND OPDForm.isdeleted=false
AND (pvar_patientname is null or LENGTH(CAST(pvar_patientname as Varchar))=0 or CAST(OPDForm.patientname as VARCHAR)=pvar_patientname)

               GROUP BY OPDForm.verifiedstatus;
