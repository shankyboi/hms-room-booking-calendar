			  ...
			  CREATE OR REPLACE FUNCTION  "get_all_MedicationType"
              (
...
,MedicationType.medicationtypedescription
			  ...
			  FROM MedicationType
			  WHERE
               (lvar_tenantid is null or COALESCE(cast(MedicationType.tenantid as varchar),'') = Any(lvar_tenantid))
                 
			   AND MedicationType.isdeleted=false
			  ;
