			  ...
			  CREATE OR REPLACE FUNCTION  "Medication_Type_List"
              (pvar_tenantid Varchar
)
			  RETURNS TABLE(tenantid uuid
,_tenantName Varchar(128)
,MedicationTypeid uuid
,medicationtypename Varchar,medicationtypedescription Varchar,createduser uuid
...
,MedicationType.medicationtypedescription
 ...
 LEFT OUTER JOIN tenant ON MedicationType.tenantid=tenant.tenantid
				...
				WHERE (lvar_tenantid is null or COALESCE(cast(MedicationType.tenantid as varchar), '') = Any(lvar_tenantid)) AND MedicationType.isdeleted=false

				 ORDER BY MedicationType.createddate DESC;
