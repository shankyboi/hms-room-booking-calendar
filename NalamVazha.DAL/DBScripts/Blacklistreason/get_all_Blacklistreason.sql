			  ...
			  CREATE OR REPLACE FUNCTION  "get_all_Blacklistreason"
              (
...
,Blacklistreason.reasondesc
			  ...
			  WHERE
               (lvar_tenantid is null or COALESCE(cast(Blacklistreason.tenantid as varchar),'') = Any(lvar_tenantid))
                 
			   AND Blacklistreason.isdeleted=false
			  ;
