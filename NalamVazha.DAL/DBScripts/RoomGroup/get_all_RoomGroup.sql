			  ...
			  CREATE OR REPLACE FUNCTION  "get_all_RoomGroup"
              (
...
,RoomGroup.groupdesc
			  ...
			  FROM RoomGroup
			  WHERE
               (lvar_tenantid is null or COALESCE(cast(RoomGroup.tenantid as varchar),'') = Any(lvar_tenantid))
                 
			   AND RoomGroup.isdeleted=false
			  ;
