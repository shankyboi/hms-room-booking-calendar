CREATE OR REPLACE FUNCTION public."Added_IPD_Application_Form"(
...
AND (COALESCE(BTRIM(pvar_bookingnumber),'')='' OR IPDApplicationForm.bookingreferencenumber ILIKE '%'||BTRIM(pvar_bookingnumber)||'%')
AND (COALESCE(BTRIM(pvar_workflowstatus),'')='' OR LOWER(BTRIM(IPDApplicationForm.bookingstatus))=LOWER(BTRIM(pvar_workflowstatus)))
AND (COALESCE(BTRIM(pvar_paymentmethod),'')='' OR EXISTS (SELECT 1 FROM billingpayment bp WHERE bp.ipdnumber=IPDApplicationForm.IPDApplicationFormid AND COALESCE(bp.isdeleted,false)=false AND LOWER(BTRIM(bp.paymentmode))=LOWER(BTRIM(pvar_paymentmethod))))
AND (COALESCE(BTRIM(pvar_financialstatus),'')='' OR LOWER(BTRIM(pvar_financialstatus))=LOWER(CASE WHEN public."Get_IPD_Remaining_Overpayment_Amount"(IPDApplicationForm.IPDApplicationFormid) > 0.009 THEN 'Over Paid' ELSE (SELECT CASE WHEN COALESCE(SUM(COALESCE(r.amount,0)),0)=0 OR COALESCE(SUM(COALESCE(r.paidamount,0)),0)=0 THEN 'Not Paid' WHEN COALESCE(SUM(COALESCE(r.paidamount,0)),0)=COALESCE(SUM(COALESCE(r.amount,0)),0) THEN 'Paid' WHEN COALESCE(SUM(COALESCE(r.paidamount,0)),0)>COALESCE(SUM(COALESCE(r.amount,0)),0) THEN 'Over Paid' ELSE 'Partially Paid' END FROM receivable r WHERE r.ipdnumber=IPDApplicationForm.IPDApplicationFormid AND COALESCE(r.isdeleted,false)=false) END))
 AND (
...
AND (
 ...
 AND COALESCE(IPDApplicationForm.verifiedstatus,'') !='Revised'
...
,_PatientProfile.registrationid as patientregistrationid
...
,CAST(COALESCE(to_char(_LatestIPScreening.appointmentdate, 'dd/MM/yyyy'), '') AS VARCHAR) AS screeningappointmentdate
,COALESCE(_LatestIPScreening.durationfrom, '') AS screeningstarttime
,COALESCE(_LatestIPScreening.durationto, '') AS screeningendtime
,COALESCE(_LatestIPScreening.doctorname, '') AS screeningdoctor
,COALESCE(_LatestIPScreening.tasktype, '') AS appointmenttasktype
,CASE
...
AND COALESCE(IPDApplicationForm_room.isdeleted,false) = false 
			    ...
			    as automaton_IPDApplicationForm_room
                    ...
                    /* Expected checkout date — latest dateofdeparture from preferred dates sub-table */
                    ,(SELECT to_char(MAX(pda.dateofdeparture), 'dd/MM/yyyy')
                      FROM IPDApplicationForm_preferreddatesofadmission pda
                      WHERE pda.IPDApplicationFormid = IPDApplicationForm.IPDApplicationFormid
                        AND COALESCE(pda.isdeleted, false) = false
                    ) AS expectedcheckoutdate
				   ...
				   ,COALESCE((
    SELECT SUM(GREATEST(COALESCE(r.amount, 0) - COALESCE(r.paidamount, 0), 0))
    FROM receivable r
    ...
    WHERE r.ipdnumber = IPDApplicationForm.IPDApplicationFormid
      ...
      AND GREATEST(COALESCE(r.amount, 0) - COALESCE(r.paidamount, 0), 0) > 0
), 0) AS requiredpendingpayment

				   ,COALESCE((
    -- Net charges against discounts/credits before deciding whether payment is pending.
    -- This matches the unified receivables page and prevents a fully adjusted bill
    -- from being reported as payable.
...
LEFT JOIN LATERAL (
    SELECT
        ...
        NULLIF(
            BTRIM(CONCAT_WS(
                ' ',
                NULLIF(_ScreeningDoctor.firstname, ''),
                NULLIF(_ScreeningDoctor.lastname, '')
            )),
            ''
        ) AS doctorname
        ,ca.tasktype
    FROM ClinicalAppointment ca
    LEFT JOIN People _ScreeningDoctor
        ON _ScreeningDoctor.Peopleid = COALESCE(ca.actualpractitioner, ca.practitioner)
    WHERE COALESCE(ca.isdeleted, false) = false
      AND LOWER(BTRIM(COALESCE(ca.origin, ''))) = 'ipd'
      AND LOWER(BTRIM(COALESCE(ca.tasktype, ''))) = 'ip screening'
     --AND LOWER(BTRIM(COALESCE(ca.status, ''))) IN ('scheduled', 'rescheduled')
      AND (
          BTRIM(COALESCE(ca.bookingid, '')) = IPDApplicationForm.IPDApplicationFormid::varchar
          OR BTRIM(COALESCE(ca.bookingid, '')) = COALESCE(IPDApplicationForm.bookingreferencenumber, '')
    )
    ORDER BY
        COALESCE(ca.modifieddate, ca.createddate) DESC,
        ca.appointmentdate DESC,
        ca.ClinicalAppointmentid DESC
    LIMIT 1
) _LatestIPScreening ON TRUE
