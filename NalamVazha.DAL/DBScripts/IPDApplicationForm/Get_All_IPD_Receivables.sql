...
AS $BODY$
    ...
    AND (
            LOWER(TRIM(COALESCE(r.receivablefor, ''))) <> 'room'

            OR EXISTS (
                SELECT 1
                FROM IPDApplicationForm_room ir
                WHERE ir.ipdapplicationformid = r.ipdnumber
                  AND ir.roomnumber = r.room
                  AND r.receivabledate::date BETWEEN ir.fromdate::date AND ir.todate::date
                  AND COALESCE(ir.isdeleted, false) = false
                 
                  AND LOWER(COALESCE(ir.action, '')) NOT IN ('inactive', 'transfer out', 'transferred out')
            )
      )
