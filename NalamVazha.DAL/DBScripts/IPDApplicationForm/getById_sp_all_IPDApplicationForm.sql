...
INNER JOIN users ON users.usersid=IPDApplicationForm_room_history.action_by
 ...
 ,(SELECT json_agg(J) FROM (
                    ...
                    ,CASE
                        WHEN lower(trim(COALESCE(reviewlogsIPDApplicationForm.bookingstatus, ''))) = 'consultation scheduled'
                        THEN COALESCE((
                            SELECT to_char(ca.appointmentdate, 'dd/MM/yyyy') || ' '
                                   || ca.durationfrom::varchar || ' - ' || ca.durationto::varchar
                            FROM ClinicalAppointment ca
                            WHERE btrim(COALESCE(ca.bookingid, '')) = IPDApplicationForm.IPDApplicationFormid::varchar
                              AND lower(trim(COALESCE(ca.origin, ''))) = 'ipd'
                              AND lower(trim(COALESCE(ca.tasktype, ''))) = 'ip new'
                              AND COALESCE(ca.isdeleted, false) = false
                              AND lower(trim(COALESCE(ca.status, ''))) <> 'cancelled'
                            ORDER BY ca.createddate DESC
                            LIMIT 1
                        ), '')
                        ELSE ''
                    END AS "Appointment On"
                    ...
                    ,CASE
                        ...
                        THEN COALESCE((
                            SELECT to_char(ca.appointmentdate, 'dd/MM/yyyy') || ' '
                                   || ca.durationfrom::varchar || ' - ' || ca.durationto::varchar
                            FROM ClinicalAppointment ca
                            WHERE btrim(COALESCE(ca.bookingid, '')) = INIPDApplicationForm.IPDApplicationFormid::varchar
                              AND lower(trim(COALESCE(ca.origin, ''))) = 'ipd'
                              AND lower(trim(COALESCE(ca.tasktype, ''))) = 'ip new'
                              AND COALESCE(ca.isdeleted, false) = false
                              AND lower(trim(COALESCE(ca.status, ''))) <> 'cancelled'
