...
CREATE OR REPLACE FUNCTION public."count_of_IPDApplicationForm"(
	pvar_tenantid character varying,
	pvar_patientname character varying,pvar_bookingstatus Varchar(1024))
    RETURNS TABLE(count bigint, verifiedstatus character varying) 
    LANGUAGE 'plpgsql'
    COST 100
...
AS $BODY$
              ...
              RETURN QUERY
			  SELECT  
			  Count(*),IPDApplicationForm.verifiedstatus 
...
AND (pvar_bookingstatus is null or LENGTH(CAST(pvar_bookingstatus as Varchar))=0 or CAST(IPDApplicationForm.bookingstatus as VARCHAR)=pvar_bookingstatus)
               GROUP BY IPDApplicationForm.verifiedstatus;
