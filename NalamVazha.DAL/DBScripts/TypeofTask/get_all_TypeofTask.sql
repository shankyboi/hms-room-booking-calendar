			  ...
			  CREATE OR REPLACE FUNCTION  "get_all_TypeofTask"
              (
...
,tenantid uuid
             ...
             if(pvar_searchterm is not null and LENGTH(CAST(pvar_searchterm as Varchar)) > 0)
			  ...
			  FROM TypeofTask
			  WHERE
               (lvar_tenantid is null or COALESCE(cast(TypeofTask.tenantid as varchar),'') = Any(lvar_tenantid))
                 
			   AND TypeofTask.isdeleted=false
			  
		        AND (((pvar_searchterm is null) or COALESCE(TypeofTask.TypeofTaskid::varchar,'') ilike pvar_searchterm) OR ((pvar_searchterm is null) or COALESCE(TypeofTask.tasktype,'') ilike pvar_searchterm) OR ((pvar_searchterm is null) or COALESCE(TypeofTask.description,'') ilike pvar_searchterm)) 
