			  ...
			  CREATE OR REPLACE FUNCTION  "get_all_ReceivableConditions"
              (
...
,COALESCE(ReceivableConditions.ismandatory,true) as ismandatory
			  ...
			  WHERE
               (lvar_tenantid is null or COALESCE(cast(ReceivableConditions.tenantid as varchar),'') = Any(lvar_tenantid))
                 
			   AND ReceivableConditions.isdeleted=false
			  ;
